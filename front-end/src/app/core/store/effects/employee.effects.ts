...
export class EmployeeEffects {
  ...
  @Effect() getProfileInfo = this.actions
    .pipe(ofType(ActionTypes.GET_PROFILE_INFO))
    .switchMap(() => this.service.getProfileInfo()
      .map(res => new GetProfileInfoSuccessAction(res))
      .catch((err) => {
        return Observable.of(new GetProfileInfoFailAction(err));
      })
    )
}